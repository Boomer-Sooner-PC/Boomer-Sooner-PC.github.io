class BoidSimulation {
    ...
    calculateBoidSeparation(boid) {
        ...
        return (
            ...
            this.calculateTurnNudge(
                boid,
                (Math.atan2(separationForce.y, separationForce.x) * 180) /
                    Math.PI,
    ...
    update() {
        ...
        for (let i = 0; i < this.boids.length; i++) {
            let nudge = 0;
            if (!isNaN(this.calculateBoidAlignment(this.boids[i]))) {
                nudge += this.calculateBoidAlignment(this.boids[i]);
            }
            if (!isNaN(this.calculateBoidCohesion(this.boids[i]))) {
                nudge += this.calculateBoidCohesion(this.boids[i]);
            }
            if (!isNaN(this.calculateBoidSeparation(this.boids[i]))) {
                nudge += this.calculateBoidSeparation(this.boids[i]);
            }
            if (!isNaN(this.calculateBoidAvoidence(this.boids[i]))) {
                nudge += this.calculateBoidAvoidence(this.boids[i]);
            }
            if (!isNaN(this.calculateBoidAttraction(this.boids[i]))) {
                nudge += this.calculateBoidAttraction(this.boids[i]);
            }

            if (isNaN(nudge) || nudge == undefined) {
                nudge = 0;
            }
